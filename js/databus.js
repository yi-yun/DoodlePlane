...
export default class DataBus {
  ...
  removeBomb(bomb) {
    let temp = (bomb === undefined) ?
      this.bomb.shift() : this.bomb.splice(this.bomb.indexOf(bomb), 1)

    temp.visible = false

    this.pool.recover('bomb', bomb)
  }
  removeBlood(blood) {
    let temp = (blood === undefined) ?
      this.blood.shift() : this.blood.splice(this.blood.indexOf(blood), 1)
